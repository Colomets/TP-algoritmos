...
bool ABCportugues(){
	...
	int vocales[]={160,133,131,198,130,136,161,162,147,228,163, 135};//Definimos el abecedario y las letras acentuadas
	int tam=26,tl=12;//Definimos el tamanio del abecedario
	show(abc, tam,4,vocales,tl);//Llamamos la funcion show para mostrar todo
	string input;
	char eleccion[2]={'a','k'};
	do{//Menu para decidir como continuar la ejecucion del programa
		...
		if(eleccion[0]!='X' or !(esUnDigito(eleccion))){
			system("cls");
			show(abc,tam,4,vocales,tl);//vuelve a mostrar toda la funcion para volver a elegir la opcion
