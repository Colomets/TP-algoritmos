...
bool ABCespaniol(){
	...
	char abc [27];//Definimos el abecedario
	for(int i=97, j=0;i<111;i++, j++) abc[j]=(char)i;
	abc[14]=(char)164;
	for(int i=111, j=15;i<123;i++, j++) abc[j]=(char)i;
	int tam=27,tl=6;//Definimos el tamanio del abecedario
	int vocales[]={160,130,161,162,163, 129};//Definimos las letras acentuadas
	show(abc, tam,1,vocales,tl);//Llamamos la funcion show para mostrar todo;
	...
	do{//Menu para decidir como continuar la ejecucion del programa
		...
		if(eleccion[0]!='X' or !(esUnDigito(eleccion))){
			system("cls");
			show(abc,tam,1,vocales,tl);//vuelve a mostrar toda la funcion para volver a elegir la opcion
